...
async function schedule(req, res) {
    ...
    if (timeRes.response.body.totalCount > 0) {
        timeData = timeRes.response.body.items.item;
    }
    else {
        timeData = false;
    }
...
{
  '오전 09:00': [ '월', '화' ],
  '정류장 - 00분 00초 _ bus.png': [ [ 127, 36 ], [ '마을:568', '마을:564' ] ],
  fire: 1,
  type: 'bus'
}
*/
async function schedule2(req, res) {
    const val = req.body;
    const keyInd = Object.keys(val);
    const sendArr = [];

    if (val.type == 'bus') {

        let url = `https://apis.data.go.kr/1613000/BusSttnInfoInqireService/getCrdntPrxmtSttnList?serviceKey=${publicKey}&_type=json&gpsLati=${val.keyInd[1][0][1]}&gpsLong=${val.keyInd[1][0][0]}`;
        const locSch = await fetch(url);
        const locRes = await locSch.json();
        const locData = locRes.response.body.items.item[0];
